...
## サービスプロバイダーとそのコスト構造の作成 {#create-service-providers-and-their-cost-structures}
...
1. 関連付けられたサービスプロバイダーのコストカテゴリと構造の定義. [詳細情報](#define-cost-categories)
...
### サービスプロバイダーとそのコストカテゴリの作成 {#create-a-service-provider-and-its-cost-categories}

#### サービスプロバイダーを追加 {#add-a-service-provider}

配信に必要な数のサービスプロバイダーを作成できます。サービスプロバイダーの追加手順は次のとおりです。

1. 次をクリック： **[!UICONTROL 新規]** ボタンを使用して、サービスプロバイダーのリストの上に表示します。
...
サービステンプレートを各サービスプロバイダーに関連付けることができるようになりました。 サービステンプレートで、まずコストカテゴリを定義し、必要に応じて、関連する在庫を定義する必要があります。その後、コスト構造を使用して、各カテゴリのコスト計算ルールを作成できます。 [詳細情報](#define-the-cost-structure)。

コストカテゴリは、ある種類の配信（E メール、ダイレクトメール、SMS など）に適した一連のコストを含むエンティティです。 コストカテゴリは、サービスプロバイダーに関連付けられているサービスのテンプレートにまとめられます。各サービスプロバイダーは、1 つ以上のサービステンプレートを参照できます。

サービステンプレートを作成し、そのコンテンツを定義するには、次の手順に従います。

1. 内 **[!UICONTROL サービス]** サービスプロバイダーのタブで、 **[!UICONTROL 追加]** ボタンをクリックし、サービステンプレートの名前を入力します。
...
1. プロセスのタイプ（ダイレクトメール／電子メールなどによる配信またはタスク）ごとにコストカテゴリを作成します。これを行うには、「**[!UICONTROL コストカテゴリ]**」タブをクリックしてから「**[!UICONTROL 追加]**」ボタンをクリックして、各コストカテゴリのパラメーターを入力します。
   ...
   * このコストカテゴリのラベルを入力し、該当するプロセスのタイプを選択します。 **[!UICONTROL ダイレクトメール]**, **[!UICONTROL 電子メール]**, **[!UICONTROL モバイル]**&#x200B;など
...
1. このコストカテゴリの値を事前に選択できます。これは、サービスプロバイダーのコストカテゴリで（空の値ではなく）デフォルトになっています。 これをおこなうには、 **はい** オプション **[!UICONTROL 選択済み]** 該当するカテゴリのタイプの列：

   ![](assets/default-cost-type.png)

   配信レベルでは、この値がデフォルトで選択されます。

### コスト構造を定義 {#define-the-cost-structure}

コストのタイプごとに、適用する計算ルールをコスト構造で指定します。
...
サービスプロバイダーに関連付けられたプロセスに関する情報を **[!UICONTROL ジョブ]** タブをクリックします。 このセクションでは、発送担当への情報の送信を設定できます。
...
サービスプロバイダーは、キャンペーン配信に関連付けられています。 配信テンプレートで参照され、このテンプレートから作成される配信でサービスを提供します。
...
配信のコストカテゴリの定義時に、コストタイプの必須選択を定義できます。 そのためには、「**[!UICONTROL コストタイプの選択を必須にする]**」を選択します。
...
1. 在庫品目の追加. [詳細情報](#add-stock-lines)。

1. アラート発生時にオペレーターに通知します。 [詳細情報](#alert-operators)。

1. オーダーして補給します。[詳細情報](#orders)。
...
在庫を作成したら、そのダッシュボードを使用して在庫品目を作成および監視します。

次をクリック： **[!UICONTROL 作成]** ボタンをクリックして、新しい在庫品目を追加します。
...
キャンペーンで、予算を選択する際は、初期金額を入力する必要があります。計算されたコストは、入力された金額（発生した費用、想定される費用、予約済みの費用、割り当てられた費用）のコミットメントレベルに応じて、自動的に更新されます。
