...
インターネットブラウザーからAdobe Campaignクライアントコンソールの画面にアクセスして、すべてのキャンペーンと配信、およびレポートとデータベース内のプロファイルに関する情報を表示できます。 このアクセスでは、レコードは作成できません。オペレーターの権限に応じて、データベース内のデータを確認したり、データに基づいて行動したりできます。例えば、キャンペーンのコンテンツとターゲティングを承認したり、配信を再開または停止することが可能です。
