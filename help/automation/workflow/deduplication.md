...
**[!UICONTROL 重複排除 - 重複]**&#x200B;アクティビティは、データセットから重複行を削除するために使用します。 例えば、以下のレコードは、同じメールアドレスと同じモバイル、または自宅の電話を持つ重複と見なすことができます。

| 最終変更日 | 名 | 姓 | メール | 携帯電話 | 電話 |
|-----|------------|-----------|-------|--------------|------|
...
1. 「**[!UICONTROL 完了]**」をクリックして、選択した重複排除メソッドを承認します。
   ...
   アクティビティエディターの下側で、グラフィカルオブジェクトのアウトバウンドトラジションのラベルを変更し、アクティビティの結果に関連するセグメントコードを入力できます。 このコードは後の手順で、ターゲティング条件として使用できます。
