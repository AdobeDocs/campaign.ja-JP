...
>[!NOTE]
>
>Adobe Campaignのweb ユーザーインターフェイスには、強力なクエリモデラーが備わっており、さまざまな条件にもとづいて特定のターゲットを選択するようにデータベースをフィルタリングするプロセスが簡素化されるため、クエリの作成と管理がより簡単になります。 Web UI のクエリモデラーについて詳しくは、[Adobe Campaign web UI ドキュメント](https://experienceleague.adobe.com/ja/docs/campaign-web/v8/query-database/query-modeler-overview){target=_blank}を参照してください。
...
1. 手順 1 で「**[!UICONTROL フィルター条件]**」を選択した場合、または&#x200B;**[!UICONTROL フィルター]**／**[!UICONTROL 詳細設定フィルター]**&#x200B;オプションを使用する場合、後からフィルター条件を手動で追加する必要があります。
   ...
   [式ビルダー](../../v8/start/filter-conditions.md#using-and--or--except)を使用して、論理オプション AND、OR、EXCEPTと組み合わせることで、さらに条件を追加することもできます。
...
この値は、ワークテーブルのスキーマです。 このパラメーターは、**[!UICONTROL tableName]** と&#x200B;**[!UICONTROL スキーマ]**&#x200B;のすべてのトランジションで有効です。
