...
Adobe Campaign には、一連のビルトインテクニカルワークフローが付属しています。 サーバー上で定期的に実行するようにスケジュールされた操作とジョブを制御します。 テクニカルワークフローでは、Campaign データベースに対するメンテナンス操作の実行、配信に関するトラッキングデータの管理、配信に関する暫定的なプロセスの設定が行われます。
...
>[!NOTE]
>
>* 各モジュールと共にインストールされるテクニカルワークフローのリストについては、[ この節 ](#list-technical-workflows) を参照してください。
>
>* Message Center アドオンに関連するテクニカルワークフローは、デフォルトでは、**[!UICONTROL 管理]**/**[!UICONTROL 実稼働]**/**[!UICONTROL Message Center]**/**[!UICONTROL テクニカルワークフロー]** ノードに格納されています。
...
Campaign テクニカルワークフローの開始および変更を行えるのは、**管理** 権限を持つオペレーターのみです。 テクニカルワークフローを監視する方法について詳しくは、[この節](monitor-technical-workflows.md)を参照してください。
