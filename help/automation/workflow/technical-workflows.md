...
この節で詳しく説明するワークフローは、別の Adobe Campaign 組み込みパッケージと共にインストールされます。これらのパッケージと関連テクニカルワークフローは、ライセンス契約によって異なります。
...
| **ffdaUnsuscribe** | デフォルトでインストール | このワークフローでは、（List-Unsubscribe メソッドを使用して）バウンスメールとして返され `<mailto>` 購読解除を処理します。 このワークフローは、エンタープライズ（FFDA）デプロイメントを持つマーケティングインスタンスに対してのみ、1 時間ごとに毎日実行されます。<br/><br/> ワークフローは、inMail モジュールによって購読解除バウンスとしてマークされた特定の時間範囲（前回の処理時間および現在の時間）からの broadlog を確認し（NmsBroadLog テーブルの iFlags 列に設定されたマーク）、broadlog のサービスが設定されているかどうかに応じて購読解除を処理します。<ul><li>serviceId が 0 （未定義）の場合、受信者はブロックリストに加えるされます。</li><li>serviceId が 0 （既存のサービスにリンク）でない場合、受信者はそのサービスの購読を解除されます。</li></ul><br/> メモ：このワークフローでは、バウンスの購読解除のみを処理します。オプトアウトリンクとワンクリックの購読解除（URL メソッド）によって行われる購読解除は、このワークフロー以外で別々に処理されます。 |
