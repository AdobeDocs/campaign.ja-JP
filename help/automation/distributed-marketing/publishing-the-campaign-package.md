...
## 他のレビュー担当者を追加 {#adding-other-reviewers}
...
リマインダーは、指定する日に送信するか、登録日の **x** 日後に送信することができます。リマインダーのタイプは、リマインダーのテーブルの最初の列で設定できます。次の例では、レビュー担当者は、01/11/2023日 ( **[!UICONTROL 日付]** 列と、承認期間の 1 日前（承認の送信日の 2 日後）に 2 つ目のリマインダーが表示されます。
