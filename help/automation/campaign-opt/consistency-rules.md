...
メッセージを配信する前に、組織が配信を処理する処理能力（物理インフラストラクチャ）、配信が生成する可能性のある応答（インバウンドメッセージ）、購読者への問い合わせに対する呼び出し数（コールセンターの処理能力）などを確保する必要があります。

それには、以下を作成します **[!UICONTROL 処理能力]** タイポロジルール。
