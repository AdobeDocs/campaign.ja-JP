...
最大負荷を定義するには、稼動ラインを定義する必要があります。それには、次の 2 つのオプションを使用できます。手動で [1 つ以上の稼動ラインを作成する](#add-availability-lines-one-by-one) または可用性の範囲を作成します。 この期間の頻度は自動化できます。 [詳細情報](#add-a-set-of-availability-lines)。
