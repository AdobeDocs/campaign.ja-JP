...
1. キャンペーンタイポロジルールのリストで、 **[!UICONTROL 新規]** アイコンをクリックします。
...
1. 次の日次再判別の実行を超えてターゲティングを保存する場合は、「**[!UICONTROL 計算パラメーター]**」セクションで頻度を定義します。[詳細情報](apply-rules.md#adjust-calculation-frequency)。
...
しきい値を定義するには、ターゲティングディメンションにリンクしたディメンションを使用できます。例えば、訪問者テーブルに保存された受信者のプロファイルに配信されるメッセージを含める ( 訪問者テーブルについて詳しくは、 （この節）(surveys/using/use-case—creating-a-refer-a-friend-form.md) を参照 )、受信者のディメンションで識別された同じ世帯に 1 週間に複数のメッセージを送信しない。
...
>[!CAUTION]
>
>しきい値の計算では、既に送信された配信のみが考慮されます。

考慮に入れる配信の期間を 2 週間に制限するには、 **15 日** 内 **[!UICONTROL 該当する期間]** フィールド：ルールが適用される配信日の最大 2 週間前に送信された配信が計算に含められます
