...
作成したタイポロジルールを適用するには、そのルールをタイポロジに関連付け、配信でこのタイポロジを参照します。
...
1. ルールを適用する配信を開きます。
1. 配信プロパティを参照し、 **[!UICONTROL タイポロジ]** タブをクリックします。
...
実際、計算によっては、毎日変更されない値を使用する場合があります。 したがって、毎日データを再計算し、データベースの負荷を増やすことは、適切とは言えません。例えば、顧客の傾向スコアと購入情報が週に一度マーケティングデータベースに保存される場合、これらの値に基づくデータを毎日再計算する必要はありません。
...
1. パーソナライゼーションの開始時に適用されるコントロールルール( ユーザーのルール（フィルター/頻度/処理能力）の有効期限が切れ、再計算が必要な場合は、この手順の間に適用されます )。
...
例えば、実行順序が 20 の頻度ルールは、実行順序が 30 の頻度ルールの前に実行されます。

### コントロールルール {#control-rules}

の場合 **[!UICONTROL 制御]** ルールを適用する場合は、どの時点で配信ライフサイクルを適用するかを決定できます。ターゲティングの前後、パーソナライゼーションの開始時、分析の終了時に、 タイポロジルールの「**[!UICONTROL 一般]**」タブにある「**[!UICONTROL フェーズ]**」フィールドで、適用する値をドロップダウンリストから選択してください。
