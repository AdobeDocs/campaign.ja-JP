...
product: campaign
title: シミュレーションの概要
description: キャンペーンのシミュレーションの設定方法を説明します
feature: Campaigns
...
で準備された配信 **テスト** モードは、相互に影響を与えません。例えば、分散型マーケティングのキャンペーンを評価する場合や、配信が暫定カレンダーにスケジュールされていない限り、影響を与えません。

したがって、頻度ルールと処理能力ルールが適用されるのは、「**[!UICONTROL ターゲットの推定とメッセージのパーソナライゼーション]**」モードの配信のみになります。配信 **[!UICONTROL 暫定ターゲットの推定と承認]** モードと **[!UICONTROL ターゲットの評価]** モードは考慮されません。
...
1. 「**[!UICONTROL 除外を調査]**」サブタブでは、結果の分析テーブルを作成できます。テーブルを作成する場合は、横軸と縦軸の変数を指定する必要があります。
   ...
   分析テーブルの作成例については、 [この節](#explore-results).
...
「**[!UICONTROL 計算]**」タブでは、ターゲットのレポートの軸を定義できます。レポートの軸は、次の期間に使用されます [結果分析](#explore-results).
