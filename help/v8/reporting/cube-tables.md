...
# キューブを使用してデータを調べる{#use-cubes-to-create-reports}

キューブを使用して、レポートを作成し、データベースからデータを識別および選択します。 以下を行うことができます。

* キューブに基づくレポートの作成。[詳細情報](#explore-the-data-in-a-report)。
* データベース内のデータを収集し、リストにグループ化することで、例えばターゲットと配信を識別して作成します。[詳細情報](#build-a-target-population)。
* レポートにピボットテーブルを挿入し、その中で既存のキューブを参照します。[詳細情報](#insert-a-pivot-table-into-a-report)。

## レポートのデータを調べる {#explore-the-data-in-a-report}

### 手順 1 - キューブに基づいてレポートを作成する {#step-1---create-a-report-based-on-a-cube}

一度 [キューブが設定されました](cube-indicators.md)に値を指定しない場合は、新しいレポートを作成するためのテンプレートとして使用できます。

既存のキューブに基づくレポートを作成するには、次の手順に従います。
...
1. ウィザ **[!UICONTROL 測定を使用]** 「 」ボタンで、既存の測定の 1 つを選択します。

   ![](assets/cube-add-measure.png)

   表示する情報と書式設定オプションを選択します。 オプションのリストは、測定のタイプに応じて異なります。
...
1. 既存の測定に基づいて新しい測定を作成できます。 それには、「**[!UICONTROL 測定を作成]**」をクリックし、測定を設定します。
   ...
   * 標準偏差：このタイプの測定の場合は、平均値と比べた各セルグループ内の偏差を計算できます。例えば、既存のすべてのセグメントの購入量を比較できます。
   ...
   測定を作成したら、その測定を編集し、設定を変更できます。 これをおこなうには、 **[!UICONTROL 測定]** ボタンをクリックし、編集する測定のタブを参照します。
...
1. 以下を使用： **[!UICONTROL 書き出し]** ボタンを使用して、買い物かご内の項目をリストにグループ化できます。
...
1. エクスポートが完了したら、実行を確認するメッセージおよび処理されたレコードの数が表示されます。
   ...
   新しいリストは、 **[!UICONTROL プロファイルとターゲット]** タブをクリックします。

   ![](assets/cube-list-available.png)

## ピボットテーブルをレポートに挿入する {#insert-a-pivot-table-into-a-report}

テーブルを作成し、キューブ内のデータを調べるには、次の手順に従います。
...
1. ページの「**[!UICONTROL データ]**」タブでキューブを選択して、そこに格納されているディメンションを処理し、計算された測定を表示します。
   ...
   これで、表示するレポートを作成できます。詳しくは、 [手順 2 — 行と列の選択](#step-2---select-lines-and-columns).
