...
1. 動的には、[列挙](../config/enumerations.md)の値に応じて、列挙に含まれる値のみを表示し、他のすべての値は「その他」にグループ化されます。 詳しくは、[bin の動的管理](#dynamically-managing-bins)を参照してください。
...
ビンを手動で作成するか、既存の[列挙](../config/enumerations.md)にリンクできます。
...
1. 「**[!UICONTROL 値を列挙に動的にリンク]**」オプションを選択し、対応する列挙を選択します。
   ...
   列挙について詳しくは、[このページ](../config/enumerations.md)を参照してください。
...
* 表示するメジャー：パーセント、合計、小数点数など[詳細情報](#changing-the-type-of-measure-displayed)
