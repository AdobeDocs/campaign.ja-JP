...
マーケティングユーザーがデータレコードにアクセスできるようにする必要があるけれど、名前（名、姓）やメールアドレスなどの受信者の個人情報（PI）を表示したくない場合は、プライバシーを保護し、正規の Campaign オペレーターがデータを誤って使用してしまうのを防ぐために、以下のガイドラインを適用します。
