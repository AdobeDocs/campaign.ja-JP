...
データパッケージの説明は、次の例のように、**xrk:navtree** データスキーマの文法に準拠する構造化XML ドキュメントです。
...
1. 「**追加**」ボタンをクリックして、パッケージとしてエクスポートするエンティティを選択します。
   ...
   >[!CAUTION]
   >
   >**[!UICONTROL オファーカテゴリ]**、**[!UICONTROL オファー環境]**、**[!UICONTROL プログラム]**&#x200B;または&#x200B;**[!UICONTROL プラン]**&#x200B;の種類のフォルダーを書き出す場合、データが失われる可能性があるため、**xtk:folder**&#x200B;を選択しないでください。 フォルダーに対応するエンティティを選択します。オファーカテゴリの場合は&#x200B;**nms:offerCategory**、オファー環境の場合は&#x200B;**nms:offerEnv**、プログラムの場合は&#x200B;**nms:program**、プランの場合は&#x200B;**nms:plan**。
...
1. 「**[!UICONTROL 次へ]**」をクリックし、抽出するドキュメントのタイプに関するフィルタークエリを定義します。 ここでは、データ抽出用のフィルタリング節を設定する必要があります。
   ...
   >[!NOTE]
   >
   >クエリエディターについては、[この節](../../automation/workflow/query.md)を参照してください。
...
インポートを円滑に行うために、エンティティパッケージはインポートする順序で並べておく必要があります。
