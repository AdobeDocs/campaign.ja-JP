...
列挙には、**Open**、**System**、**Emoticon**&#x200B;または&#x200B;**Closed**&#x200B;を指定できます。
...
列挙フィールドでは、値をドロップダウンリストから選択するか、リストで使用できない場合は手動で入力できます。 **[!UICONTROL 開く]** オプションが有効になっている場合、カスタム値を列挙に追加できます。 これらの値は、後でエイリアスのクレンジングによって標準化され、バリエーションが正しい用語に自動的に置き換えられます（例えば、`Adob`を`Adobe`に変換します）。


>[!CAUTION]
>
>データクレンジングは、データベース値に影響を与える重要な操作です。 Adobe Campaignでは、データの一括更新が実行されます。これにより、特定の値が削除される場合があります。 この操作は、エキスパートユーザーのみを対象としています。
...
* **-updateHits:full**&#x200B;を使用して、すべてのエイリアス ヒットを再計算します。
