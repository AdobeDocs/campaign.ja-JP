...
description: Campaign の推奨される安全な設定ガイダンス
feature: Privacy, PI
role: Developer
level: Beginner
...
### このガイダンスの参照先 {#public-guidance}

Adobe Campaignは現在、推奨されるセキュリティで保護された設定ガイダンスを、機械で読み取り可能な形式で提供していません。 次のドキュメントを使用して、現在の設定と推奨される安全なデフォルト設定を比較できます。

* **このページ** - [Campaign セキュリティのベストプラクティス ](#ac-security) （チェックリストと詳細な節）
* **[Campaign 設定（FAQ）](../start/campaign-faq-comprehensive.md#settings)** – 設定を、推奨される安全なデフォルトと比較します
* **[強化されたセキュリティアドオン](enhanced-security.md)** - セキュア CMK 統合とセキュア VPN トンネリング
* **[権限の基本を学ぶ](../start/gs-permissions.md)** - アクセスプロファイルと製品プロファイル
* **[PII 表示の制限](../dev/restrict-pi-view.md)** – 機密フィールドへのアクセスの制限
* **[実装ガイドライン](../start/implement.md)** – 開始前のセキュリティとプライバシー
