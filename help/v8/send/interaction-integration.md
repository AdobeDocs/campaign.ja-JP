...
# Web ページにオファーを追加する{#add-an-offer-in-web}

Web ページでオファーエンジンを呼び出すには、ページに JavaScript コードを直接挿入します。 この呼び出しにより、ターゲット要素のオファーコンテンツが返されます。
...
HTMLページには、作成したオファースペースの内部名の値 (「i_internal name space」) を持つ@id属性を持つ要素を含める必要があります。 オファーは、インタラクションによってこの要素に挿入されます。
...
この静的呼び出しは、オファーエンジンで必要なすべてのパラメーターを含む動的呼び出しを自動的に生成します。

この動作により、同じページに複数のオファースペースを使用して、1 回のオファーエンジン呼び出しで管理できます。

**手順 3:結果をHTMLページに表示**

オファー表示域のコンテンツは、オファーエンジンによってHTMLページに返されます。
...
識別された連絡先に対してオファーを提示する手順は、詳細な手順と似ています [この節](#presenting-an-anonymous-offer).

Web ページのコンテンツに、オファーエンジンの呼び出し中に連絡先を識別する次のスクリプトを追加する必要があります。
...
1. Web ページが表示される間、スクリプト評価を使用して、受信者 ID をオファーエンジンに渡すことができます。 ID が複合キーの場合は、詳細設定で使用されたのと同じ順番に並んだキーが | で区切られて表示されます。
   ...
   次の例では、連絡先は、Web サイトにログオンし、オファーエンジンの呼び出し中に E メールと名前によって認識されます。
...
### HTMLレンダリング関数の使用 {#using-an-html-rendering-function}
...
### オファーを提示 {#presenting-an-offer}

Campaign **インタラクション** モジュールを使用すると、オファーエンジンを呼び出したHTMLページに XML ノードを返すことができます。 この XML ノードは、顧客側で開発した関数で処理できます。
...
以下の使用例では、XML モードを有効にしてからエンジン呼び出しの結果をHTMLページに表示するために、Adobe Campaignで実行する設定について詳しく説明します。

1. **環境とオファースペースの作成**

   環境の作成について詳しくは、 [このページ](interaction-env.md).

   オファースペースの作成について詳しくは、 [このページ](interaction-offer-spaces.md).
...
1. **オファーの作成**
   ...
   オファーの作成について詳しくは、 [このページ](interaction-offer.md).
...
### レンダリング関数を使用 {#using-a-rendering-function-}

XML レンダリング関数を使用して、オファーのプレゼンテーションを作成できます。この関数は、オファーエンジンの呼び出し中にHTMLページに返される XML ノードを変更します。
