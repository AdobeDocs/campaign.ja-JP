...
定義済みフィルターが既に作成されている場合は、ユーザーフィルターのリストから選択できます。 [詳細](interaction-predefined-filters.md)
...
1. 「**[!UICONTROL OK]**」をクリックして重み付けを保存します。
   ...
   >[!NOTE]
   >
   >選択されたオファーに関して、複数の重み付けの実施要件を満たすターゲットには、それらのうち最大の重み付けが適用されます。オファーエンジンの呼び出し時に、1 つのオファーが選択される回数は、1 つのコンタクト先につき最大 1 回です。
