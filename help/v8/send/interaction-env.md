...
デフォルトでは、Campaign には、受信者テーブル（識別されたオファー）をターゲットとする組み込みの環境が 1 つ用意されています。匿名プロファイルのインバウンドインタラクション用の訪問など、別のテーブルをターゲットにするには、設定を更新する必要があります。
...
例えば、受信者環境のオファースペース（識別されたコンタクト先）と訪問者環境に合ったオファースペース（識別されていないコンタクト先）をリンクすると、これにより、連絡先が識別されているかどうかに応じて、様々なオファーが連絡先に提供されます。 詳しくは、[オファースペースの作成](interaction-offer-spaces.md)を参照してください。
