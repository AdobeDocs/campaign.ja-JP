...
>
>* 新しいプロバイダーへの接続の設定には、技術的なスキル、TCP に関する知識、バイナリ、16 進表現、およびテキストエンコーディングが必要になる場合があります。 プロバイダーとのアクティブな協力も求められます。
...
* **受信者サーバー、ポート、アカウント、パスワード、システム タイプ**
これらの設定は、送信機+受信機モードの場合に受信機に適用されます。 トランスミッター部分と同様に機能します。詳しくは、[上記](#smpp-connection-settings)を参照してください。
* **ログ ファイルで詳細SMPP トレースを有効にする**
...
メッセージのデフォルトのソースアドレスを定義します。 この設定は、ソース番号が配信内で空のままの場合にのみ適用されます。 デフォルトでは、ソース番号フィールドは渡されないので、プロバイダーはショートコードの代わりにソース番号フィールドを使用します。
...
このオプションを選択すると、上記の正規表現で処理されるテキストにオプションフィールドのコンテンツが追加されます。 テキストの形式は「0xTAG:VALUE」で、0xTAGは大文字（0x002Eなど）のタグの4桁の16進数値です。
...
Campaignでは、MO （nms:inSms テーブル）の3つの追加フィールド（リンクされたSMS、エイリアス、および大きなアカウント）を受信できます。 SMPP コネクタを使用すると、これらのフィールドには、任意のオプション SMPP パラメーター（TLV）から取得したデータを任意の共通形式で入力できます。
...
>[!NOTE]
>
>**顧客アカウント** フィールドが空のままになっている場合、短いコードに置き換えられます。
