...
また、強制隔離リストからアドレスを手動で削除することもできます。強制隔離からアドレスを手動で削除するには、ステータスを **[!UICONTROL 有効]** に変更します（**[!UICONTROL 管理/キャンペーン管理/配信不能件数の管理/配信不能件数およびアドレス]** ノードから）。

![](assets/tech-quarantine-status.png)

### 一括更新 {#unquarantine-bulk}

受信者に正常に配信できないためにメールが誤ってバウンスとしてマークされる ISP の停止など、特定の状況では、強制隔離リストで一括更新を実行する必要が生じる場合があります。

一括更新を実行するには：

1. ワークフローを作成し、強制隔離テーブル（**[!UICONTROL nms:address]**）にクエリを追加して、影響を受けた受信者をフィルタリングします
2. 次のようなクエリ条件を使用して、強制隔離を解除するアドレスを識別します。
   * **電子メールドメイン（@domain）** は、影響を受ける ISP ドメインと同じです
   * **更新ステータス（@lastModified）** 機能停止の期間内
   * **ステータス（@status）** が強制隔離ステータスと等しい
3. **[!UICONTROL データを更新]** アクティビティを追加して、アドレスのステータスを **[!UICONTROL 有効]** に設定します

その後、アドレスは **[!UICONTROL データベースクリーンアップ]** ワークフローによって強制隔離リストから自動的に削除され、今後の配信に含めることができます。
