...
* **ハードバウンス**
ハードバウンスとは、ISPが利用者のメールアドレスに電子メールを送信しようとし、配信不能と判定した際に生成される永続的なエラーです。 Adobe Campaign 内では、配信不能と分類されたハードバウンスは強制隔離リストに追加され、再試行されません。 エラーの原因が不明な場合、ハードバウンスが無視されることがあります。

  ハードバウンスの一般的な例：アドレスが存在しない、無効なアカウント、無効な構文、無効なドメイン

* **ソフトバウンス**
ソフトバウンスとは、ISPが電子メールの配信に問題が発生した場合に生成される、一時的なエラーのことです。 ソフトエラーは、配信を成功させる試みとして、複数回（カスタム配信設定または標準の配信設定を使用するかによって異なります）[再試行](#retries)されます。 継続的にソフトバウンスを行うアドレスは、再試行の最大回数に到達するまで強制隔離に追加されません（設定によって異なります）。
...
* **同期エラー**: Adobe Campaign配信サーバーから連絡を受けたリモート サーバーは、直ちにエラーメッセージを返します。 配信をプロファイルのサーバーに送信することはできません。 メール転送エージェント（MTA）は、バウンスのタイプを決定してエラーを検証し、その情報を Campaign に返して、関係するメールアドレスを強制隔離する必要があるかどうかを判断します。 [バウンスメールの選定](#bounce-mail-qualification)を参照してください。
...
* **非同期エラー**：非同期の配信エラーを検証するために Campaign で使用されるルールは、**[!UICONTROL 管理／キャンペーン管理／配信不能件数の管理／配信ログ選定]**&#x200B;ノードに一覧表示されます。 非同期バウンスは、**[!UICONTROL 受信メール]** ルールを通じてinMail プロセスによって選定されます。
...
+++ クリックすると、メールエラータイプの完全なリストが表示されます
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> ソフト </td> 
   <td> 5 </td> 
   <td> このユーザーのメールボックスはいっぱいになっていて、メッセージをこれ以上受け入れることができません。 このプロファイルは、エラーカウントが 5 にならない限り、再びターゲットになります。 その後、レコードは強制隔離ステータスに設定され、以降は再試行されなくなります。<br /> この種類のエラーはクリーンアップ プロセスによって管理されます。アドレスは30日後に有効な状態に設定されます。<br /> 警告：アドレスを強制隔離アドレスのリストから自動的に削除するには、データベース クリーンアップ テクニカル ワークフローを開始する必要があります。<br /> </td> 
  </tr> 
  <tr> 
   <td> 未接続 </td> 
   <td> 無視 </td> 
  ...
  <tr> 
   ...
   <td> ソフト／ハード </td> 
   <td> 3 </td> 
   <td> メッセージ配信チェーンでエラーが発生しました。 SMTP リレーのインシデント、一時的に到達できないドメインなどです。エラーに従って、エラーカウンターが5に達するまでアドレスが再試行されるか、強制隔離に直接送信されます。<br /> </td> 
  </tr> 
  <tr> 
   <td> 不明なユーザー </td> 
   <td> ハード </td> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td> 証明書の問題（パスワード、破損など） APNs問題<br />への接続をテストします </td> 
   <td> 失敗<br /> </td> 
   <td> エラーによってエラーメッセージが異なる<br /> </td> 
   <td> ソフト<br /> </td> 
   <td> 拒否<br /> </td> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td> メッセージの作成／分析フェーズ：ペイロードが大きすぎる<br /> </td> 
   <td> 失敗<br /> </td> 
   <td> 通知が重すぎます：{1} ビットです。ただし、{2}のみが許可されています<br /> </td> 
   <td> ソフト<br /> </td> 
   <td> 拒否<br /> </td> 
   <td> ×<br /> </td> 
  </tr> 
  <tr> 
   <td> 送信中にネットワーク接続が切断<br /> </td> 
   <td> 失敗<br /> </td> 
   <td> アドレス {1}<br />のFirebase Cloud Messaging サービスからの応答がありません </td> 
   <td> ソフト<br /> </td> 
   <td> 未到達<br /> </td> 
   <td> ○<br /> </td> 
  </tr> 
...
+++ クリックして、標準コネクタのSMS エラータイプを表示します
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td> MT 確認が無効<br /> </td> 
   <td> 失敗<br /> </td> 
   <td> 送信クエリ <br />の確認応答フレームを処理中にエラー'{1}'が発生しました </td> 
   <td> ソフト<br /> </td> 
   <td> 未到達<br /> </td> 
  </tr> 
  <tr> 
...
このセクションでは、一般的な配信エラーの問題の診断と解決に関するガイダンスを提供します。

### パーソナライゼーションエラーによる失敗したステータス {#personalization-errors}

メール配信のステータスが&#x200B;**[!UICONTROL 失敗]**&#x200B;である場合は、パーソナライゼーションブロックの問題に関係している可能性があります。 配信のパーソナライゼーションブロックは、スキーマが配信マッピングと一致しない場合にエラーを生成する可能性があります。

配信ログは、配信が失敗した理由を知るうえで重要です。 次のような一般的なエラーが発生する可能性があります。
...
一部の配信は、「未到達」エラーで失敗する可能性があります。

```
Inbound email bounce (rule 'Auto_replies' has matched this bounce).
```

**説明**：これは、配信が成功したが、Adobe Campaignが受信者から自動返信を受け取ったこと（例：「不在」の返信）が「自動返信」の受信メールルールに一致することを意味します。

自動返信メールはAdobe Campaignによって無視され、受信者のアドレスは強制隔離に送信されません。 これは期待される動作であり、配信エラーを示すものではありません。
