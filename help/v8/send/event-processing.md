...
* **送信済み**、**無視**&#x200B;および&#x200B;**配信エラー**
これらの配信ステータスは、**updateEventsStatus** ワークフローを介して復元されます。 詳細を確認するには、該当する配信を開きます。
* **該当しないイベント**
トランザクション メッセージのルーティング フェーズが失敗しました。 たとえば、Adobe Campaign がこのイベントのテンプレートとなるメールを見つけられなかった場合などです。
* **イベント期限切れ**
送信試行回数の上限に達しました。 このイベントは空とみなされます。
