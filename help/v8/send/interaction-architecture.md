...
これら 2 種類のインタラクションは、**単一モード**（オファーが 1 つのコンタクトに対して計算される）か&#x200B;**バッチモード**（オファーが一連のコンタクトに対して計算される）のいずれかで実行できます。通常、インバウンドインタラクションは単一モードで実行され、アウトバウンドインタラクションはバッチモードで実行されます。とは言え、例えば[トランザクションメッセージ](transactional.md)のような例外もあり、その場合アウトバウンドインタラクションは単一モードで実行されます。
