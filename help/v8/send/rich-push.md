...
1. から **[!UICONTROL 通知タイプ]** ドロップダウンで次を選択 **[!UICONTROL デフォルト]**.
...
1. プッシュ通知をさらにパーソナライズするには、 **[!UICONTROL 通知オプション]** および **[!UICONTROL HTTPv1 追加オプション]** プッシュ通知の。 [詳細情報](#push-advanced)
...
1. から **[!UICONTROL 通知タイプ]** ドロップダウンで次を選択 **[!UICONTROL 基本]**.
...
1. の下 **[!UICONTROL カラーオプション]** メニューで、の 16 進数カラーコードを入力します **[!UICONTROL タイトル]**, **[!UICONTROL メッセージ]** および **[!UICONTROL 背景・経緯]**.

1. を追加 **[!UICONTROL 「後で通知」ボタン]** 必要に応じて、 を入力 **[!UICONTROL リマインダーテキスト]** および **日付** 対応するフィールド

   この **[!UICONTROL リマインダー日]** フィールドには、秒でエポックを表す値が想定されます。
...
1. 「」を選択します **[!UICONTROL リンクタイプ]** ボタンのリンクされた URL の：
...
1. から **[!UICONTROL 通知タイプ]** ドロップダウンで次を選択 **[!UICONTROL カルーセル]**.
...
1. 方法を選択 **[!UICONTROL カルーセル]** 次のように操作します。

   * **[!UICONTROL 自動]**：画像をスライドとして自動的に切り替えさせ、定義済みの間隔で移行します。
   * **[!UICONTROL 手動]**：ユーザーはスライド間を手動でスワイプして画像間を移動できます。

1. から **[!UICONTROL レイアウト]** ドロップダウンで次を選択 **[!UICONTROL フィルムストリップ]** 前の画像と次の画像のプレビューをメインスライドの横に含めるオプション。

1. クリック **[!UICONTROL 画像を追加]** 画像 URL、テキスト、アクション URL を入力します。
...
| **[!UICONTROL アイコン]** | プロファイルのデバイスに表示する通知のアイコンを設定します。 |
...
| **[!UICONTROL リンクタイプ]** | <ul><li>Web URL: Web URL は、ユーザーをオンラインコンテンツに誘導します。 クリックすると、デバイスのデフォルトの web ブラウザーが開き、指定した URL に移動するように求められます。</li><li>ディープリンク：ディープリンクは、アプリが閉じられた場合でも、アプリ内の特定のセクションへとユーザーをガイドする URL です。 クリックすると、ダイアログが表示され、ユーザーはリンクを処理できる様々なアプリから選択できます。</li><li> アプリを開く：アプリの URL を開くと、アプリケーション内のコンテンツに直接接続できます。 これにより、アプリケーションでは、曖昧さ回避ダイアログをバイパスして、特定のタイプのリンクのデフォルトハンドラーとして確立できます。</li></ul> |
...
| **[!UICONTROL 通知の優先度]** | 通知の優先度レベル（デフォルト、最小、低、高）を設定します。 優先度レベルは通知の重要度と緊急性を決定し、通知の表示方法と、特定のシステム設定をバイパスできるかどうかに影響します。詳しくは、[FCM のドキュメント](https://firebase.google.com/docs/reference/fcm/rest/v1/projects.messages?hl=ja#notificationpriority)を参照してください。 |
