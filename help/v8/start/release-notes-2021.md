...
* 時間コンポーネントの抽出中にAmazon Redshift HoursDiff関数とMinutesDiff関数を実行すると、誤った結果が返される問題を修正しました。（NEO-31673）
...
* トラッキング署名機能が改善され、サードパーティツール（メールクライアント、インターネットブラウザーなど）にリンクされたエラーを防ぐことができるようになりました。 特殊文字を処理します。 URL パラメーターがエンコードされるようになりました。
