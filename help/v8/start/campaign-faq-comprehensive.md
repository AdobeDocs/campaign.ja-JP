...
**注意：** オンプレミスデプロイメントまたはハイブリッドデプロイメントが必要な場合、Campaign Classic v7 は引き続きフルサポートで使用できます。
...
* **ワークフローログ** – 問題のトラブルシューティング用に詳細な実行ログにアクセスできます
* **ヒートマップ** - ワークフローアクティビティを視覚化し、パフォーマンスボトルネックを特定します
* **[監査記録](../reporting/audit-trail.md)** - ワークフローに加えられたすべての変更を追跡します
* **アラート** - ワークフローの失敗または遅延の通知を設定します
