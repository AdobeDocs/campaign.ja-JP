...
+++ オペレーターと権限がセキュリティのベストプラクティスに従っていることを確認する方法

Adobe Campaign v8 では、最上位の管理アカウントと権限のあるアカウントの現在のすべての設定を、推奨される安全なデフォルト設定と比較できます。

* **現在の設定（完全な表示）:** 管理者アカウントと権限のあるアカウントのすべての設定は、（管理 **&#x200B;**&#x200B;> **[!UICONTROL アクセス管理]** （クライアントコンソールまたは Campaign web UI）で確認できます。例：**[!UICONTROL オペレーター]**、**[!UICONTROL オペレーターグループ]**、**[!UICONTROL ネームド権限]** （管理、承認管理など）、フォルダーレベルの権限。 また、製品管理者は、[Campaign Campaign コントロールパネル許可リストに加える &#x200B;](https://experienceleague.adobe.com/docs/control-panel/using/control-panel-home.html?lang=ja){target="_blank"} でインスタンスレベルの設定（IP）や、URL 権限、インスタンスの詳細などを確認することもできます。
* **推奨されるセキュアなデフォルト：** セキュリティガイドライン [&#128279;](../config/security.md) は、管理者アカウントと特権アカウントに推奨されるセキュアなベースラインを定義します。これには、十分なセキュリティグループの作成、オペレーターごとの適切なアクセス権限の割り当て、admin オペレーターと admin グループの使用の制限が含まれます。 該当する場合、ネットワークと暗号化の堅牢化については、[&#x200B; セキュリティの強化アドオン &#x200B;](../config/enhanced-security.md) も参照してください。
* **比較：** 管理者は、現在の構成と文書化されたこれらの推奨事項を比較して、ギャップを特定し、安全なデフォルトと一致させます。

**関連トピック：**

[&#x200B; 権限の概要 &#x200B;](gs-permissions.md) | [&#x200B; セキュリティガイドライン &#x200B;](../config/security.md) | [&#x200B; セキュリティ強化アドオン &#x200B;](../config/enhanced-security.md)
...
Campaign v8 では、Federated Data Access （FDA）が、Campaign データモデルを変更せずに、外部データベースに保存されたデータをクエリして使用できるようになりました。 外部アカウントを設定し、ワークフロー、クエリおよびターゲティングで FDA を使用して、外部データを Campaign プロファイルと組み合わせます。

FDA の可用性と接続要件は、デプロイメントとライセンスによって異なります。 互換性マトリックスで、サポートされるデータベースと必要なバージョンを確認します。

**関連トピック：**

[FDA 接続の設定 &#x200B;](../connect/fda.md) | [&#x200B; 互換性マトリックス &#x200B;](compatibility-matrix.md)
...
クライアントコンソールを閉じ、ユーザープロファイルからローカルキャッシュファイルを手動で削除します。

**手動でのキャッシュのクリーンアップ（Windows）:**

1. クライアントコンソールで、**[!UICONTROL ファイル]**/**[!UICONTROL ローカルキャッシュをクリア…]** を選択します。
2. ログアウトして、クライアントコンソールを閉じます。
3. Windows のバージョンに応じて、キャッシュの場所に移動します。 Windows 10 の場合：`C:\Users\<Username>\AppData\Roaming\Neolane\NL_5\`
4. `nlclient-config-<alphanumerical>.xml` ファイルおよび関連するフォルダを削除します。

>[!IMPORTANT]
>
>`nlclient_cnx.xml` を削除しないでください。

Adobe サポートは、お使いのバージョンおよび OS の正しいキャッシュの場所を確認し、安全な手動クリーンアップをガイドします。

詳しくは、[&#x200B; クライアントコンソールのインストールと設定 &#x200B;](connect.md) を参照してください

+++

+++ Adobe Campaign を LDAP と統合できますか？ 

いいえ。Campaign v8 は LDAP 統合をサポートしていません。 認証はAdobe IMS（Adobe IDまたはFederated ID）を使用します。 Campaign Classic v7 から移行する場合は、サポートされていない機能と認証に関する変更を確認します。

**関連トピック：**

[Campaign v8 への接続 &#x200B;](connect.md) | [v8 でサポートされていない機能 &#x200B;](v7-to-v8.md#gs-removed)
