...
アーキテクチャ、デプロイメント、移行パス、機能の変更など、Campaign v8 と以前のバージョン（Classic v7 および Standard）の主な違いについて説明します。
...
* **メリット：** インフラストラクチャ管理なし、自動拡張、エンタープライズクラスのセキュリティ

**アーキテクチャとパフォーマンス：**

* **v8:** PostgreSQL データベースを使用した拡張フル FDA （FFDA）アーキテクチャ
* **v8:** 数百万のプロファイルの処理と大量の送信に最適化
* **v8:** クエリのパフォーマンスとデータ処理の速度が大幅に向上
* **メリット：** 大規模な運用および複雑なキャンペーンのパフォーマンスが向上

**ユーザーインターフェイス：**

* **v8:** クライアントコンソールと共に、新しい Campaign web ユーザーインターフェイスが追加されました
* **v8:** ユーザーエクスペリエンスが向上した、最新のレスポンシブデザイン
* **v8:** キャンペーンの作成と管理のワークフローを簡素化
* **メリット：** オンボーディングの迅速化、キャンペーンの容易な実行、アクセシビリティの向上

**アップグレードとメンテナンス：**

* **v8:** Adobeで管理される自動アップグレード – 常に最新の安定したバージョンを使用
* **v7:** 手動のアップグレードの計画と実行が必要
* **メリット：** メンテナンス負担の軽減、新機能への迅速なアクセス、ダウンタイムなし

**API と統合：**

* **v8:** パフォーマンスと信頼性が向上した最新の REST API
* **v8:** Adobe Experience Cloud ソリューションとのシームレスな統合
...
* [Campaign Standard から v8 へ](acs-to-v8.md)
...
* **大量のキャンペーン** - パフォーマンスと信頼性を向上させて何百万ものメッセージを送信します。
...
* **クラウドネイティブのメリット** – 自動更新、インフラストラクチャ管理、柔軟な拡張を活用
...
* 他のAdobe Experience Cloud ソリューションとのシームレスな統合が必要です
* 大規模なアップグレードまたはインフラストラクチャの更新を計画しているとします
* **将来性の高いテクノロジが求められている** - Campaign Classic v7 とCampaign Standardはサポート終了となり、v8 を戦略的な長期ソリューションにします。
* **チームには最新のインターフェイスが必要です** – 新しい Campaign web UI は、マーケターがより直感的でアクセスしやすいエクスペリエンスを提供します

**移行に関する考慮事項：**

* 自動移行はまだ利用できません – Adobeが移行のサポートとガイダンスを提供します。
* v8 は Managed Cloud Serviceのみです（オンプレミスデプロイメントまたはハイブリッドデプロイメントは不可）
* 一部の技術実装は v7 と異なる場合があります。[&#x200B; 互換性マトリックス &#x200B;](compatibility-matrix.md) を確認してください。
...
* [Campaign Classic v7 から v8 へ &#x200B;](v7-to-v8.md) - v7 ユーザー向けの詳細な比較および移行ガイド
* [Campaign Standardから v8 へ &#x200B;](acs-to-v8.md) – 標準ユーザーの移行パス
* [Campaign v8 機能マトリックス](../start/compatibility-matrix.md)

+++

+++ v8 で異なる、または使用できないCampaign Classic v7 の機能には何がありますか？

Campaign v8 では、Campaign Classic v7 のほとんどの機能に機能強化が加えられていますが、クラウドネイティブアーキテクチャによって一部の機能が変わりました。
...
* **ビルドアップグレード** – 手動ではなく自動（Adobe管理）になりました
...
詳細については、[&#x200B; 互換性マトリックス全体を確認 &#x200B;](compatibility-matrix.md) してください。

**関連トピック：**

* [ガードレール](ac-guardrails.md)
