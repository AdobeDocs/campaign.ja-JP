...
1. 配信チャネルを選択します。 そのためには、ドロップダウンリストから適切な配信テンプレートを選択します。
   ...
   組み込みテンプレートは、インストールした各チャネル（電子メール、電話、モバイルチャネル（プッシュ/SMS）、ダイレクトメール、X （Twitter）など）に対して提供されます。リストで使用可能なチャネルは、ライセンス契約によって異なります。
...
1. 「**[!UICONTROL 追加]**」ボタンをクリックし、制限フィルターを定義します。
   ...
   * **[!UICONTROL 条件のフィルタリング]**：このオプションを使用して、カスタムクエリを定義し、受信者を取得します。 クエリのデザインについて詳しくは、[この節](../start/query-editor.md)を参照してください。
...
>[!VIDEO](https://video.tv.adobe.com/v/3447000?captions=jpn&quality=12)
...
1. **プルーフを送信**：この手順では、コンテンツ、URL、パーソナライゼーションなどを制御できます。[詳細情報](../send/preview-and-proof.md)。
