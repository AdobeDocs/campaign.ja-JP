---
title: メールトラッキングピクセルとCNIL ガイダンス
...
2026年4月14日、フランスのデータ保護機関であるCommission nationale de l&#39;informatique et des libertés （CNIL）は、メール内でのトラッキングピクセルの使用に関する[推奨事項](https://www.cnil.fr/sites/default/files/2026-04/recommandation-pixels_de_suivi.pdf)を公開しました。 このガイダンスでは、同意が必要なタイミングを明確にし、メールのピクセル追跡における適切な同意管理の重要性を強調しています。 このポリシーは、フランスに拠点を置く購読者にメールを配信するエンティティの送信方法に影響を与える可能性があります。
...
* **配信の分類。** emailType属性（認証、配信品質のみ、トランザクション、マーケティング、パブリックサービス、B2B見込み顧客）を使用してnms:deliveryを拡張します。 この分類により、同意なしに許可されるピクセルが決まります。
* **同意キャプチャ。** 文言のバージョン、タイムスタンプ、キャプチャソース、および有効期限を含む目的ごとの同意構造を使用してnms:recipientを拡張します。 サインアップフォームとプリファレンスセンターを拡張して、メールオプトインとは別にピクセル同意を収集します。
* **ピクセルの放出。** ピクセルの目的ごとに1つのNmsTracking_OpenFormulaを定義します（認証、配信品質、パフォーマンス、プロファイリング、不正検知）。 配信タイポロジルールは、emailTypeと受信者の目的ごとの同意に基づいて、出力する数式を選択します。 パーソナライゼーションブロックはロジックをカプセル化し、個々のクリエイターには存在しないようにします。
* **引き出し。** 「トラッカーの管理」環境設定リンクを、すべてのメールフッターに追加します。このリンクは、登録解除リンクとは異なります。 このリンクは、idTrackingで認証されたnms:webApp ランディングページを指します。受信者は、電子メールアドレスを再入力することなく、ワンクリックで同意を取り消します。 標準のトラッキングワークフローに追加されたフィルターステップは、以前に配信されたメールの再開後に悪用されるのを防ぎます。
* **同意確認。** 追加のみのログ（例えば、pix:consentLog拡張機能の名前空間）内の各同意イベントをキャプチャし、文言の変更後の検索性のために文言バージョンを個別に保存します。 Adobe Campaign エクスプローラーを介して、定期的な書き出しとしてログを表示します。
