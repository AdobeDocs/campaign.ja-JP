...
[Campaign 互換性マトリックス](capability-matrix.md)を参照してください。
...
* 「BCC で E メールを送信」（ブラインドカーボンコピー）機能を有効にして、配信プロパティの専用オプションを使用して、Campaign から送信された E メールを配信レベルで保存できるようになりました。 [詳細情報](../config/email-settings.md#email-bcc)
* パフォーマンスを向上させるために、ルーティング外部アカウントで、新しい「分割」オプションがデフォルトで有効になりました。 このオプションを使用すると、受信者にすばやく配信できるように、メッセージをミッドソーシングインスタンス間で自動的に分割できます。 リンク
* ミッドソーシング設定の LINE 配信で、同じタイプの複数のアクティブなアカウントがミッドインスタンスに存在できるようになりました。
