...
* タイポロジルールで定義されているメッセージの最大数に達した場合に配信の準備が失敗する問題を修正しました。 （NEO-40062）
* E メールアドレスに「s」文字が含まれている場合に、Adobe Analyticsコネクタの設定中に発生していた問題を修正しました。 （NEO-39082）
* ポストアップグレード中に、deliveryMapping テーブルがカスタム配信マッピングからデータを失う可能性がある問題を修正しました。 （NEO-40133）
* E メールアドレスに一重引用符 (&#39;) が含まれている場合に、同じ配信で受信者が同じメッセージを複数回受け取る可能性がある問題を修正しました。 この文字はエスケープされました。 （NEO-41198）
* シードまたは代替アドレスを使用した配達確認を送信する際の ID 生成の問題を修正しました。 （NEO-42670）
* アドレス方式の置き換えを使用して配達確認を送信できない場合がある問題を修正しました。 （NEO-40417）
