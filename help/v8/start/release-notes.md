...
* [ 動的レポート ](https://experienceleague.adobe.com/docs/campaign-web/v8/reports/dynamic-reporting/get-started-reporting.html){target="_blank"} （オンデマンドのみ）
* [ ブランディングの一元化 ](https://experienceleague.adobe.com/docs/campaign-web/v8/conf/branding/branding-gs.html){target="_blank"}
...
* ユーザーの介入なしに大規模アップグレードを容易にするクライアントコンソールのサイレントインストールメカニズムを提供しました。 これにより、手動でのインストールに関する課題を解決できます。 （NEO-69772）
...
* 出力時間が正しくないロシア（UTC+3:00 モスクワ）のタイムゾーンの不一致 `nlserver` 問題を解決しました。 時間同期のロジックが更新されました。 （NEO-74754）
