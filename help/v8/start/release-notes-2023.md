...
description: 2023 Campaign v8 リリースに付属する機能と改善点の一覧
...
詳しくは、[詳細ドキュメント](../send/push-data-collection.md)を参照してください。
...
* データベース クリーンアップ ワークフローがFFDAのnms:address テーブルのアドレスを削除できない問題を修正しました。 （NEO-54460）
...
* Adobe Analytics コネクタで、正しいチャネルタイプを使用して指標をエクスポートするようになりました。 以前は、常に「メール」チャネルとして設定されていました。 （NEO-26340）
...
* 重複を避けるために、**[!UICONTROL 統合トラッキング]** （nms:trackingStats） FFDA テーブルで統合トラッキングデータの管理が改善されました。 （NEO-46409）
